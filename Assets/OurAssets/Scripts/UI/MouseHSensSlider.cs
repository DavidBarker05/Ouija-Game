...
public class MouseHSensSlider : MonoBehaviour
{
    [SerializeField]
    MouseHSensInputField m_SensInput;
    ...
    void OnEnable()
    {
        ...
        float sens = UserSettingsManager.Instance?.MouseHorizontalSensitivityMultiplier ?? 1f;
    ...
    public void ChangeSens(float value)
    {
        ...
        m_SensInput.InputField.text = sens.ToString("F1");
        UserSettingsManager.Instance.MouseHorizontalSensitivityMultiplier = sens;
        m_SensInput.InputField.onSubmit.AddListener(m_SensInput.ChangeSens);
    }
}
